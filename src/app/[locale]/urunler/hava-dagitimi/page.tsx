...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = dict.products.havaDagitimi;
      ...
      <section className="relative bg-white">
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            {t.alpaca.subModels.map((model: string) => (
              <div
                key={model}
                className="group relative overflow-hidden rounded-xl bg-gray-50 px-4 py-3.5 ring-1 ring-gray-100 transition-all duration-300 hover:bg-white hover:shadow-md hover:ring-primary/20"
              >
                <div className="absolute left-0 top-0 h-full w-0.5 bg-gray-200 transition-colors duration-300 group-hover:bg-primary" />
                <p className="text-[13px] font-semibold text-secondary/65 transition-colors duration-300 group-hover:text-dark">
                  {model}
                </p>
              </div>
            ))}
