  ...
  return (
    <main>
      ...
      <section className="relative bg-white">
        <span className="pointer-events-none absolute right-0 top-0 select-none text-[24rem] font-black leading-none text-gray-50" aria-hidden="true">01</span>
        <div className="relative mx-auto max-w-7xl px-4 py-28 sm:px-6 lg:px-8">
          <div className="mb-14 flex items-end gap-6">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.25em] text-primary">{t.roo.label}</p>
              <h2 className="mt-2 text-4xl font-extrabold tracking-tight text-dark sm:text-5xl">ROO</h2>
              <p className="mt-4 max-w-lg text-[15px] leading-7 text-secondary/55">
                {t.roo.subModels.length} {t.roo.desc}
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {t.roo.subModels.map((model: string, i: number) => (
              <div key={model} className="group relative overflow-hidden rounded-xl bg-gray-50 px-5 py-4 ring-1 ring-gray-100 transition-all duration-300 hover:bg-white hover:shadow-md hover:ring-primary/20">
                <div className="absolute left-0 top-0 h-full w-0.5 bg-gray-200 transition-colors duration-300 group-hover:bg-primary" />
                <div className="flex items-center gap-4">
                  <span className="text-xl font-black text-secondary/[0.06]">{String(i + 1).padStart(2, "0")}</span>
                  <p className="text-sm font-semibold text-secondary/65 transition-colors duration-300 group-hover:text-dark">{model}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-12 flex flex-wrap gap-3">
            ...
            <a href="tel:+902164674752" className="inline-flex items-center gap-2 rounded-xl border border-gray-200 px-7 py-4 text-sm font-bold text-secondary transition-all duration-300 hover:border-primary/30 hover:text-primary">
              +90 216 467 47 52
            </a>
          </div>
        </div>
