      ...
      <section className="bg-gray-50 py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            {/* Vertical line */}
            <div className="absolute left-[18px] top-2 bottom-2 hidden w-px bg-gray-200 sm:block" />

            <div className="space-y-3">
              {t.serviceSteps.items.map((item: { step: string; phase: string }, i: number) => (
                <div key={i} className="group flex items-start gap-5">
                  {/* Number circle */}
                  <div className="relative z-10 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white text-[12px] font-bold text-secondary/40 ring-2 ring-gray-200 transition-all duration-300 group-hover:ring-primary group-hover:text-primary">
                    {String(i + 1).padStart(2, "0")}
                  </div>

                  {/* Content */}
                  <div className="flex flex-1 items-center gap-4 rounded-xl bg-white px-6 py-4 ring-1 ring-gray-100 transition-all duration-300 group-hover:shadow-lg group-hover:ring-primary/15">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {t.whyImportant.items.map((item: { title: string; text: string }, i: number) => (
              <div
                key={i}
                className="group relative overflow-hidden rounded-xl bg-gray-50 p-6 ring-1 ring-gray-100 transition-all duration-300 hover:bg-white hover:shadow-lg hover:ring-primary/15"
              >
                <div className="absolute left-0 top-0 h-full w-1 bg-gray-200 transition-colors duration-300 group-hover:bg-primary" />
                <span className="mb-3 block text-2xl font-black text-primary/12">
                  {String(i + 1).padStart(2, "0")}
                </span>
