...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const t = dict.technical.dokumanKutuphanesi;
      ...
      <section className="relative flex min-h-[420px] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-14 pt-32 sm:px-6 lg:px-8">
          <nav className="mb-8 flex items-center gap-2 text-xs text-white/40">
            ...
            <span>/</span>
            <Link href={`/${locale}/teknik-merkez/dokuman-kutuphanesi`} className="transition-colors hover:text-white/70">{t.breadcrumb.technicalCenter}</Link>
            <span>/</span>
      ...
      <section className="bg-gray-50 py-20">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-primary">{t.documentTypes.sectionLabel}</p>
              <h3 className="mt-1 text-xl font-bold text-dark">{t.documentTypes.title}</h3>
            </div>
            <div className="hidden h-px flex-1 bg-gray-200 sm:block" />
          </div>
          <div className="grid gap-4 sm:grid-cols-3">
            {t.documentTypes.items.map((doc: { title: string; description: string }) => (
              <div key={doc.title} className="group relative overflow-hidden rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-100 transition-all duration-300 hover:shadow-lg hover:ring-primary/20">
                ...
                <h4 className="mt-4 text-sm font-bold text-dark">{doc.title}</h4>
                <p className="mt-1.5 text-xs leading-relaxed text-secondary/45">{doc.description}</p>
                <p className="mt-4 text-[11px] font-semibold text-primary opacity-0 transition-opacity duration-300 group-hover:opacity-100">{t.documentTypes.comingSoonLabel}</p>
              </div>
            ))}
          </div>
        </div>
